...
try {
  ...
  if (isset($_POST['caracteristica_delete_id'])) {
    if (empty($_POST['caracteristica_delete_id'])) {
      exit("Nulo");
    }
  } else {
    exit("No hay POST");
  ...
  $caracteristica->setpk($_POST['caracteristica_delete_id']);
  echo $caracteristica->delete();
} catch (Exception $th) {
